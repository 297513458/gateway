package cn.gateway.core.common;
...
public class KeyUtil {
	 ...
	 * @return
	 */
	public static final boolean verifyCheckword(String checkword, String checkwordDencrypt) {
		try {
	...
	public static final String passwordEncrypt(String message) {
		return changeWord(MD5Util.MD5Encode(MD5Util.MD5Encode(changeWord(MD5Util.MD5Encode(message)))));
	}
	...
	private static String disploseKey(String key, int length) {
		try {
			key = AESUtil.byte2hex(StringUtils.trimToEmpty(key).getBytes("utf-8"));
		} catch (Exception e) {
		}
		if (key == null)
			key = "";
	 ...
	 * @return
	 */
	public static final boolean verifyPassword(String password, String passwordEncrypt) {
	...
	public static final String commonEncode(String message, String key) {
		try {
			if (key != null && message != null && key.trim().length() > 0 && message.trim().length() > 0)
